...
type OrderByItem<Doc extends DocumentData, Key = keyof Doc> = OrderByArray<Doc> | Key | KeyHack;
type OrderByType<Doc extends DocumentData> = OrderByItem<Doc> | OrderByArray<Doc>[];
...
export const getDocs = async <Data extends DocumentData, Doc extends Document = Document<Data>>(
  path: string,
  // queryString: string = '{}',
...
const createFirestoreRef = <Doc extends DocumentData>(
  ...
  if (orderBy) {
    if (typeof orderBy === "string") {
      ...
      const multipleOrderBy = (o: OrderByType<Doc>): o is OrderByArray<Doc>[] => {
        return Array.isArray((o as OrderByArray<Doc>[])[0]);
      };
      (multipleOrderBy(orderBy) ? orderBy : [orderBy]).forEach(([order, direction]) =>
...
const createListenerAsync = async <Doc extends Document = Document>(
  ...
  return new Promise((resolve) => {
    ...
    const ref = createFirestoreRef(path, query);
    const unsubscribe = onSnapshot(ref, { includeMetadataChanges: true }, (querySnapshot) => {
      const data: Doc[] = [];
      querySnapshot.forEach((doc) => {
        const docData =
          doc.data({
...
export type CollectionSWROptions<Doc extends Document = Document> = SWRConfiguration<Doc[] | null>;
/**
 ...
 */
export const useCollection = <Data extends DocumentData, Doc extends Document = Document<Data>>(
  path: string | null,
  ...
  const memoQueryString = useMemo(
    ...
    [endAt, endBefore, isCollectionGroup, limit, orderBy, startAfter, startAt, where]
  );
  ...
  const swr = useSWR<Doc[] | null>(
    ...
    async (path: string, queryString: string) => {
      if (shouldListen.current) {
        ...
        const { unsubscribe, initialData } = await createListenerAsync<Doc>(path, queryString, {
      ...
      const data = await getDocs<Doc>(path, JSON.parse(queryString) as CollectionQueryType<Doc>, {
        parseDates: dateParser.current,
        ignoreFirestoreDocumentSnapshotField: shouldIgnoreSnapshot.current,
      });
      return data;
